 ...
 *   handler 整体兜底。上游还有第二层：它 await 路由 handler 并把 rejection 接住
 *   （记日志 + 回 400，见 dsh-host-webserver 的分发处），所以这里的 try/catch 是加固
 *   而不是唯一防线——但别因此松懈，那层只覆盖「handler 返回的 promise 被拒」，覆盖不到
 *   注册期（apply 里）抛出的异常。
